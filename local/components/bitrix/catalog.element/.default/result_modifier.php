<?php
/**
 * result_modifier.php — bitrix:catalog.element / chokerz
 *
 * Подготовка данных $arResult перед передачей в template.php.
 * Вся бизнес-логика — здесь. Шаблон — только вывод.
 *
 * Что делает:
 *  1. Формирует галерею из DETAIL_PICTURE + свойства MORE_PHOTO
 *  2. Нормализует цену и валюту
 *  3. Строит плоский список уникальных цветов и размеров из офферов (SKU)
 *  4. Готовит JSON офферов для клиентского JS
 *  5. Формирует характеристики для вкладки «Характеристики»
 *  6. Строит JSON-LD Schema.org Product
 *  7. Готовит флаги бейджей, наличия, маркетплейс-ссылок
 *
 * Изменения (2026-02-24):
 *  - Начальная версия для детальной страницы товара
 *
 * @package CHOKERZ
 * @version 1.0
 */
